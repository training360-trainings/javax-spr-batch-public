...
//                         Step echoStep,
    ...
    @Bean
    public Flow loadFlow(Step copyFileStep,
                         Step loadOrdersStep,
                         Step notifyStep,
                         JobExecutionDecider jobExecutionDecider
                         ) {
        return new FlowBuilder<SimpleFlow>("loadFlow")
                .start(copyFileStep)
                .next(loadOrdersStep)
                .next(jobExecutionDecider)
                .on("PARTIALLY_COMPLETED").to(notifyStep)
...
//                .retryLimit(5)
    ...
    @Bean
    public Step notifyStep(Tasklet notifyTasklet) {
