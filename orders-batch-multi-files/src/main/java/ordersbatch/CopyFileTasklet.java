...
//                    .put("file", outputPath.toString());
            ...
            Files.list(inputDir)
                    .filter(file -> file.getFileName().toString().startsWith("orders-" + date))
                    .forEach(path -> {
                        try {
                            Files.copy(path,
                                    stagingDir.resolve(path.getFileName().toString()));
                        } catch (IOException e) {
                            throw new IllegalStateException("Can not copy file " + path, e);
                        }
                    });
